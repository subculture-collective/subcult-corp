...
var import_sdk, log, OPENROUTER_API_KEY, DEFAULT_MODELS, LLM_MODELS, _client, OLLAMA_BASE_URL, OLLAMA_TIMEOUT_MS, OLLAMA_MODEL;
...
`,
      patch_code: (ctx) => `Read the relevant source files from projects/ using file_read.
...
`,
      convene_roundtable: (ctx) => `This step triggers a roundtable conversation.
The payload should specify the format and topic.
Provide a summary of what the roundtable should discuss and why.
`,
      propose_workflow: (ctx) => `Based on the payload, propose a multi-step workflow.
...
function createFileWriteExecute(agentId) {
  return async (params) => {
    ...
    const path = rawPath.replace(/\.\.\//g, "");
    const relativePath = path.startsWith("/workspace/") ? path.replace("/workspace/", "") : path;
...
var spawnDroidTool = {
  ...
  execute: async (params) => {
    ...
    const outputFilename = params.output_path ?? "output.md";
